...
const fromWei = (value: number) => ethers.utils.formatEther(typeof value === "string" ? value : value.toString());
...
describe("Exchange", () => {
  ...
  describe("addLiquidity", async () => {
    describe("empty reserves", async () => {
      ...
      it("does not allow zero amounts", async () => {
        ...
        await expect(exchange.addLiquidity(0, 0, DEADLINE, { value: 0 })).to.be.revertedWith("need more than 1000000000 wei to create pool");
  ...
  describe("removeLiquidity", async () => {
    ...
    it("burns LP-tokens", async () => {
      await expect(() => exchange.removeLiquidity(toWei(25), toWei(25), toWei(50), DEADLINE)).to.changeTokenBalance(exchange, owner, toWei(-25));
