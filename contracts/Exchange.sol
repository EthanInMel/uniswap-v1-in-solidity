...
contract Exchange is ERC20, IExchange {
  ...
  |__________________________________*/
    ...
    function ethToTokenOutput(
        ...
        require(
            deadline >= block.timestamp && tokens_bought > 0 && max_eth > 0
        );
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 eth_sold = getOutputPrice(
            tokens_bought,
            address(this).balance - max_eth,
            token_reserve
        );
    ...
    function tokenToTokenSwapInput(
        ...
        address payable exchange_addr = payable(
            factory.getExchange(token_addr)
        );
        return
            tokenToTokenInput(
                tokens_sold,
    ...
    function tokenToTokenOutput(
        ...
        require(
            deadline >= block.timestamp &&
                (tokens_bought > 0 && max_eth_sold > 0)
        );
        require(exchange_addr != address(this) && exchange_addr != address(0));
        uint256 eth_bought = IExchange(exchange_addr).getEthToTokenOutputPrice(
            tokens_bought
        );
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 tokens_sold = getOutputPrice(
        ...
        require(token.transferFrom(buyer, address(this), tokens_sold));
        uint256 eth_sold = IExchange(exchange_addr).ethToTokenTransferOutput{
            value: eth_bought
